...
void BattleUI::Update(BattleContext& ctx, float dt){
    ...
    case BattleState::WaitingForInput:
        if(ctx.currentTurn == BattleSide::Player){
            ...
            selectedSkill = GetRandomValue(0, 3);
            if(ctx.enemy.active().skills[selectedSkill].cooldown > 0){
                selectedSkill = -1;
                ctx.state = BattleState::WaitingForInput;
            }
            else ctx.state = BattleState::SelectingSkill;
    ...
    case BattleState::SwitchingCharacter:
        ...
        for(int i = 0; i < 3; i++){
            ...
            Rectangle rec = layout.switchCharacter;
            rec.x = layout.switchCharacter.x + (20 + layout.switchCharacter.width) * r;
            layout.switchCharacterPressed = (IsMouseButtonPressed(MOUSE_BUTTON_LEFT) && CheckCollisionPointRec(GetMousePosition(), rec)) || (IsKeyPressed(KEY_A + r));
            if(layout.switchCharacterPressed){
                ctx.pendingAction.bActType = BattleActionType::Switch;
                ctx.player.activeIndex = i;
                ctx.state = BattleState::ExecutingSkill;
            }
             else if((IsMouseButtonPressed(MOUSE_BUTTON_LEFT) && !CheckCollisionPointRec(GetMousePosition(), rec)) || IsKeyPressed(KEY_S)) ctx.state = BattleState::WaitingForInput;
            r++;
        }
    }
}
    ...
    if(ctx.state == BattleState::WaitingForInput || ctx.state == BattleState::SelectingSkill || ctx.state == BattleState::SwitchingCharacter){
        if(showPlayerDependantUi){
            for(int i = 0; i < 4; i++){
                ...
                rec.x = layout.skillButton.x + (20 + layout.skillButton.width) * i;
                if(ctx.state == BattleState::SelectingSkill && i == selectedSkill) rec.y -= (rec.height + 10.0f);
...
void BattleScene::ExecuteAction(){
    if(battle.pendingAction.bActType == BattleActionType::Switch){
        ...
        else if(battle.pendingAction.skill->target == "enemy_single") battle.pendingAction.target = &battle.enemy.active();
